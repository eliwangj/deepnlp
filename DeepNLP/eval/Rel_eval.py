...
def semeval_official_eval(preds, labels, outdir):
    ...
    eval_cmd = "perl {}/semeval2010_task8_scorer-v1.2.pl {} {}".format(eval_path,proposed_answer, answer_key)
    print(eval_cmd)
    p,r,f1 = 0,0,0
    try:
        ...
        # res = [s for s in os.popen(eval_cmd).read().split("\n") if "The official score" in s][0]
        # print(res)
        # f1 = float(re.findall('F1 = (.+?)%',res)[0])
...
def tacred_official_eval(preds, golds, output_dir=None):
    # pred_labels = [label_map[id] for id in preds]
    # gold_labels = [label_map[id] for id in golds]
    preds = ['no_relation' if l.lower() == "other" else l for l in preds]
    golds = ['no_relation' if l.lower() == "other" else l for l in golds]
    ...
    if output_dir is not None:
        ...
        with open(proposed_answer, 'w', encoding='utf-8') as f:
            for idx, pred in enumerate(preds):
                f.write("{}\t{}\n".format(idx, pred))
        with open(answer_key, 'w', encoding='utf-8') as f:
            for idx, pred in enumerate(golds):
                if pred == 'other':
                    pred = 'Other'
                f.write("{}\t{}\n".format(idx, pred))

    prec_micro, recall_micro, f1_micro = score(golds, preds)
    return {
...
def measure_statistics(preds, labels, rel_size, ignore_label):
    ...
    GPU & CPU code
    """
    # ignore_label = IGNORE_LABEL[task]

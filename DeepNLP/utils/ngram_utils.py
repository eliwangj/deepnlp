...
class Find_Words:
    def __init__(self, ngram_length=5, min_count=10, min_pmi=0):
        ...
        for text in self.text_filter(texts):
            ...
            for i in range(len(text) - 1):
                if text[i:i + 2] in self.strong_segments:  # 如果比较“密切”则不断开
                    ...
                    s = text[i + 1]
        self.words = {i: j for i, j in self.words.items() if j >= self.min_count and self.ngram_length >= len(i)}
...
def dlg(all_sentences, ngram_length, min_freq, dlg_threshold):
    ...
    for gram, c_gram in n_gram_dict.items():
        ...
        dlg = c_gram * np.log10(c_gram) + corpus_size * np.log10(corpus_size) - new_corpus_size * np.log10(
            new_corpus_size)
        if dlg > max_dlg_2:
            max_dlg_2 = dlg
        if dlg < min_dlg_2:
    ...
    new_dlg_dict_2 = {gram: c for gram, c in new_dlg_dict.items() if c >= min_freq and len(gram) <= ngram_length}
...
def pmi(all_sentence, ngram_length, min_freq, pmi_threshold):
    all_sentences = []
    for sentence in all_sentence:
        all_sentences.append(''.join(sentence))

    fw = Find_Words(ngram_length, min_freq, pmi_threshold)
...
def av(all_sentences, ngram_length, min_freq, av_threshold=5):
    n_gram_dict = {}
    new_all_sentences = []
    ...
    for sentence in new_all_sentences:
        for i in range(len(sentence)):
            for n in range(1, 1+ngram_length):
                if i + n > len(sentence):
                    break
                left_index = i - 1
                right_index = i + n
...
def extract_ngram(all_sentences, ngram_length=5, min_feq=0):
    n_gram_dict = {}
...
def renew_ngram_by_freq(all_sentences, ngram2count, min_feq, ngram_len):
    new_ngram2count = {}
...
def get_ngram2id(all_sentence, ngram_length=5, threshold=0, flag='pmi',
                 av_threshold=5, pmi_threshold=0, dlg_threshold=0):

    if flag == 'dlg':
        ngram2count = dlg(all_sentence, ngram_length, threshold, dlg_threshold)
    elif flag == 'pmi':
        ngram2count = pmi(all_sentence, ngram_length, threshold, pmi_threshold)
    elif flag == 'av':
        ngram2count = av(all_sentence, ngram_length, threshold, av_threshold)
    else:
        raise ValueError()

    ngram2count = renew_ngram_by_freq(all_sentence, ngram2count, threshold, ngram_length)

    gram2id = {'<PAD>': 0}
    index = 1
    for word, count in ngram2count.items():
        gram2id[word] = index
        index += 1
    return gram2id, ngram2count

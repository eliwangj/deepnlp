...
class DSnt(nn.Module):
    ...
    def load_data(self, data_path=None, sentence_list=None, aspect_list=None):
        if data_path is not None: # if reading a data file
            flag = data_path[data_path.rfind('/')+1: data_path.rfind('.')]
            lines = readfile(data_path, flag)
        elif sentence_list is not None: # if passed into samples
            flag = 'predict'
            # >----------------- #
            if aspect_list is not None:
                if isinstance(aspect_list[0][0], int): # index form
                    aspect_words = []
                    for i in range(len(aspect_list)):
                        index_tuple = aspect_list[i]
                        first_word = sentence_list[i][index_tuple[0]]
                        second_word = sentence_list[i][index_tuple[1]]
                        aspect_words.append((first_word, second_word))
                    lines = [(s, a, i, '0',  None, None) for s, a, i in zip(sentence_list, aspect_words, aspect_list)]

...
class DRel(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if self.bilstm is not None:
            ...
            pdb.set_trace()
            '''
            sequence_output, _ = self.bert(input_ids.cuda(), token_type_ids.cuda(), attention_mask.cuda(), output_all_encoded_layers=False)
    ...
    def predict(self, sentence_list, e1_list, e2_list, eval_batch_size=16):
        ...
        for start_index in tqdm(range(0, len(eval_examples), eval_batch_size), colour='green', unit='sentence', unit_scale=eval_batch_size):
            ...
            #print("input_ids:",type(input_ids))
            #print("ngram_ids:",type(ngram_ids))
            #print("segment_ids",type(segment_ids))
            #print("input_mask",type(input_mask))
            '''input_ids: <class 'torch.Tensor'>
            ngram_ids: <class 'NoneType'>
            segment_ids <class 'torch.Tensor'>
            input_mask <class 'torch.Tensor'>
            
            print("entity_mark:",type(entity_mark))
            entity_mark: <class 'torch.Tensor'>
            pdb.set_trace()
            '''

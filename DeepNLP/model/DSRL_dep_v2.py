...
class DSRL(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, valid_ids=None,
                attention_mask_label=None, batch_examples=None, verb_index=None, labels=None,
        ...
        batch_size, _, feat_dim = sequence_output.shape # record shape 啥是_?
        ...
        for i in range(batch_size):
            try:
                ...
                print("------------- IndexError Info -------------")
                print("i", i)
                print("verb_index: ", verb_index)
                print("verb_index[i][0]: ", verb_index[i][0])
                print("Sentence: ", batch_examples[i].text_a)
                print("target_verb: ", batch_examples[i].text_a[int(verb_index[i][0]) - 1])
        ...
        if labels is not None:
            if self.crf is not None:
                ...
                loss = -1 * self.crf(emissions=s_labels, tags=labels, mask=attention_mask_label)
                return loss
            else:
            ...
            if self.crf is not None:
                ...
                predictions = self.crf.decode(s_labels, attention_mask_label)[0]
                return predictions
            else:
        ...
        for indx, (example, tokens, labels, valid, label_mask, eval_mask) in \
            ...
            for i in range(len(labels)):
                if labels[i] in self.labelmap: # if label can be identified
                    ...
                    label_ids.append(self.labelmap['_'])  # unknown label
    ...
    ### ------------------------------------------------ ###
        ...
        logger.info("***** Arguments and Parameters *****")
        logger.info(vars(args))   # print out all the arguments and parameters
    ...
    ### ------------------------------------------------ ###
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"): # starting training from here, one epoch at a time
            ...
            for step, start_index in enumerate(tqdm(range(0, len(train_examples), args.train_batch_size), colour='green', unit='sentence', unit_scale=args.train_batch_size)): #遍历每个数据
                sr_tagger.train()
                ...
                loss = sr_tagger(input_ids, segment_ids, input_mask, valid_ids, l_mask, # 输入bert里面拿到一个loss
                                batch_examples=batch_examples, verb_index=verb_index, labels=labels,
    ...
    ### ------------------------------------------------ ###
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:  # 把前面的重复了一遍 但是不会做gradient descent
                prediction = {flag: [] for flag in eval_data.keys()} # eval_data.keys() are dev and test
                logger.info('\n')
                for flag in eval_data.keys():
                    ...
                    for start_index in range(0, len(eval_examples), args.eval_batch_size):
                        ...
                        with torch.no_grad():
                            pred = sr_tagger(input_ids, segment_ids, input_mask, valid_ids, l_mask,
                                             batch_examples=batch_examples, verb_index=verb_index, labels=None,
                                             input_ngram_ids=ngram_ids, ngram_position_matrix=ngram_positions)  # predicted results

                        lens = l_mask.sum(1).tolist()
                        all_pred.extend(pred[l_mask].split(lens)) # .split(lens) to seperate different sentences (of a batch). what has been added to all_pred here are a few (batch size) tensors whose values are the keys of label_map


                    # 这里可能有问题，dep srl应该加'_'而不是'O'。这会增加False Positives(FP)，即本来是没有argument的，你预测它有。【是不是也会增加相同数量的FN？】
                    # label_map[0] = 'O'  # add 'O' to label_map
                    label_map[0] = '_'  # single underscore indicates a non-valued position
                    ...
                    convert_back(prediction[flag], gold_filepath, predict_filepath) # a function to convert the predictions back into the official tabular form
                    # ------------------ #
                    ...
                    evalscript_path = '/data/wangjunqiang/dnlptk-main-sep2/DeepNLP/eval/srlconll-1.1/bin/eval_09.pl'
...
def get_label(train_data_path): # get a list of all types of labels 
    ...
    label_list = ['_']  # initialize the list with a non-valued token
...
def convert_back(all_pred, gold_filepath, predict_filepath): # convert the predictions back into the official tabular form
    with open(gold_filepath, 'r') as infile, open(predict_filepath, 'w') as outfile:

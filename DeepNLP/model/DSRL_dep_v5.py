...
class DSRL(nn.Module):
    ...
    def __init__(self, labelmap, hpara, model_path, sense2id_dic, verb2sense_dic, emb_word2id=None):    # Usage: Initialize the class DSRL
        ...
        self.num_labels = len(self.labelmap) + 1            # plus one because of '_'
        ...
        self.sense_embedding = nn.Embedding(len(self.sense2id_dic), self.hpara['feat_dim_sense'], padding_idx=0)  # Initialize an Embedding module for senses, containing (#senses + 1) tensors of size 200. padding_idx=0 means we need an all-zero embedding for padding index (here is 0).
        # -----------------< #
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, valid_ids=None,
                attention_mask_label=None, batch_examples=None, verb_index=None, labels=None, gold_sense_ids=None,
        ...
        batch_size, _, feat_dim = sequence_output.shape # _ is hidden parameter
        ...
        sense_pad_length = max(map(len, list(self.verb2sense_dic.values())))  # maximum amount of possible senses for a predicate
        for i in range(batch_size):
        ...
        # [batch_size, seq_len, n_labels]se
        s_labels = self.srl_attn(arg_h, pre_h).permute(0, 2, 1)

        # >----------------- #
        if labels is not None: # if training
            ## given gold_sense_ids, we want the position of gold ids in sense_candidates. e.g. if sense_candidates = [[5, 0, 0], [6, 103, 67], ...]], gold_sense_ids = [5, 67, ...], we want [0, 2, ...]
            gold_senses = []
            for i in range(len(gold_sense_ids)):
                gold_sense_id = gold_sense_ids[i]
                possible_senses = sense_candidates[i]
                if gold_sense_id in possible_senses:
                    position = possible_senses.index(gold_sense_id)
                    gold_senses.append(position)
                else: # normally this won't happen
                    print("Error: gold_sense_id {} not in possible_senses {}".format(gold_sense_id, possible_senses))
                    gold_senses.append(0)
            gold_senses = torch.tensor(gold_senses, device=valid_output.device) # convert the list into a tensor and put it on device

            if self.crf is not None:
                arg_loss = self.crf(emissions=s_labels, tags=labels, mask=attention_mask_label)
                sense_loss = self.loss_function(product_scores, gold_senses) # 如果有，报错(RuntimeError: CUDA error: device-side assert triggered)是这一行导致的
                crf_loss = (-1 * arg_loss) + sense_loss
                return crf_loss
            ...
            predicted_ids = []  # pure list
            for i in range(len(product_scores)):
                ## if this predicate only has one sense candidate
                if sum(id != 0 for id in sense_candidates[i]) == 1:
                    nonzero_idx = [i for i, id in enumerate(sense_candidates[i]) if id != 0] # the index of the nonzero(thus, valid) element
                    sense_id = sense_candidates[i][nonzero_idx[0]]  # get the id of the sense
                    predicted_ids.append(sense_id)
                
                ## else this predicate has more than one candidates
                else:
                    t = product_scores[i]
            ...
            if self.crf is not None:
                return self.crf.decode(s_labels, attention_mask_label)[0], predicted_ids
            else:
                print('self.crf is None')
                ...
                return pre_labels, predicted_ids
        # -----------------< #
    ...
    ### ------------------------------------------------ ###
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"): # starting training from here, one epoch at a time
            ...
            for step, start_index in enumerate(tqdm(range(0, len(train_examples), args.train_batch_size), colour='green', unit='sentence', unit_scale=args.train_batch_size)): #遍历每个数据，one batch at a time
                sr_tagger.train()
                ...
                # >----------------- #  get gold sense_ids; [1, 12, 8, 30, 4, 9, ...] length is batch_size
                gold_sense_ids = []
                for example in batch_examples:
                    sense = example.sense_label[0]  # sense_label is in the form: ['发现.01']
                    sense_id = get_sense_id(sense, sense2id_dic)
                    gold_sense_ids.append(sense_id)
                # -----------------< #
                ...
                loss = sr_tagger(input_ids, segment_ids, input_mask, valid_ids, l_mask, # training mode: get a loss from BERT
                                batch_examples=batch_examples, verb_index=verb_index, labels=labels, gold_sense_ids=gold_sense_ids,
    ...
    ### ------------------------------------------------ ###
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:  # 把前面的重复了一遍 但是不会做gradient descent
                ...
                for flag in eval_data.keys():
                    ...
                    for start_index in range(0, len(eval_examples), args.eval_batch_size):
                        ...
                        with torch.no_grad():
                            arg_pred, sense_pred = sr_tagger(input_ids, segment_ids, input_mask, valid_ids, l_mask,  # evaluation mode: get predictions
                                             batch_examples=eval_batch_examples, verb_index=verb_index, labels=None, gold_sense_ids=gold_sense_ids,
                                             input_ngram_ids=ngram_ids, ngram_position_matrix=ngram_positions) 
                    ...
                    label_map[0] = '_'  # add '_' to label_map
...
class InputExample(object): # the objects of InputExample are used in load_data function
    ...
    def __init__(self, guid, text_a, text_b=None, labels=None, verb_index=None, sense_label=None):
        ...
        Args:
            ...
            sense_label: (Optional) The label of sense of this single current predicate, e.g. ['发现.01']
        """
        self.guid = guid
        self.text_a = text_a
        self.text_b = text_b
...
def get_sense2id(train_data_path): # get a dictionary where keys are unique ids and values are corresponding predicate senses
    sense2id = {0: '<PAD>'}     # 0 is the index for padding
    index = 1
    ...
    for line in lines:  # Indenting back ensures that I don't hold the file open longer than necessary.
        ...
        if splits[12] == 'Y':  # '_' should also have an index
            if sense not in sense2id:
                ...
                index += 1
    return sense2id     # {0: '<PAD>', 1: 'predict.01', 2: 'predict.02', ...}
# -----------------< #
...
def get_possible_sense_ids(target_verb, sense2id_dic, verb2sense_dic): # given the target verb, we want to find the possible sense ids it corresponds to
    ...
    try:
        ...
        sense_ids.append(0) # if the current target_verb (in dev/test dataset) is not in verb2sense_dic, add padding index 0
    return sense_ids
# -----------------< #

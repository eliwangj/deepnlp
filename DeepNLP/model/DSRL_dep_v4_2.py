...
class DSRL(nn.Module):
    ...
    def __init__(self, labelmap, hpara, model_path, sense2id_dic, verb2sense_dic, emb_word2id=None):    # Usage: Initialize the class DSRL
        ...
        self.sense_embedding = nn.Embedding(len(self.sense2id_dic) + 1, self.hpara['feat_dim_sense'])  # Initialize an Embedding module for senses, containing (#senses + 1) tensors of size 200. plus 1 b/c padding
        # -----------------< #
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, valid_ids=None,
        ...
        for i in range(batch_size):
            try:
                ...
                print("verb_index[i][0]: ", verb_index[i][0] )
                import pdb; pdb.set_trace()
        ...
        if labels is not None: # if training
            ...
            gold_senses = torch.tensor(gold_senses, device=valid_output.device) # convert the list into a tensor

            if self.crf is not None:
                # print('self.crf is not None')
                arg_loss = self.crf(emissions=s_labels, tags=labels, mask=attention_mask_label)
                sense_loss = self.loss_function(product_scores, gold_senses) # 报错(RuntimeError: CUDA error: device-side assert triggered)是这一行导致的
    ...
    ### ------------------------------------------------ ###
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:  # 把前面的重复了一遍 但是不会做gradient descent
                ...
                for flag in eval_data.keys():
                    ...
                    # sense2id_dic[0] = '&'  # if & appears, the prediction is wrong as the inner product between 0 and target predicate is larger
...
# >-------仍需改进---------- #  如果dev或者test里面出现verb2sense_dic里没有的词，就append(0)吗? 有没有更好的选择？

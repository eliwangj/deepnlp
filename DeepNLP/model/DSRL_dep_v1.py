        ...
        ### ------------------------------------------------ ###
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:  # 把前面的重复了一遍 但是不会做gradient descent
                ...
                for flag in eval_data.keys():
                    ...
                    # ------------------ #
                    output_filepath = os.path.join(output_model_dir, '')
                    gold_filepath = ''
                    convert_back(all_pred, output_filepath) # a function to convert the predictions back into the official tabular form
                    # ------------------ #
                    ...
                    eval_path = '/data1/junqiang/dnlptk-main-sep2/DeepNLP/eval/srlconll-1.1/bin/eval_09.pl'
                    command = 'perl ' + eval_path + ' -g %s -s %s' % (gold_filepath, output_filepath) # define the command that executes the eval script
...
def convert_back(all_pred, output_filepath): # convert the predictions back into the official tabular form
    return ''
